...
starter:
  index.html: |
    ...
    <body>
      ...
      <p>Allergy information comes from the FDA:
        <a href="www.fda.gov/food">the nine major food allergens</a></p>
...
Every restaurant has to tell people what is in the food. The FDA keeps the official list
of the nine ingredients that cause most serious allergic reactions, so that is a genuinely
useful thing to link your menu to.

```html
<a href="https://www.fda.gov/food/nutrition-food-labeling-and-critical-foods/food-allergies">the nine major food allergens</a>
```

That is a long address, and all of it matters. Leave off the `https://` and the browser
will hunt for a *file* in your own folder instead.
...
2. The second is meant to leave your site and go to the FDA, but the address does not
   start with `https://`. Press Try it and click it. The browser will look for a *file*
   called `www.fda.gov/food` in your own folder, not find one, and give you an error.
   The whole address it needs is:
